...
export function PlatformPageClient() {
  return (
    <main className="min-h-screen bg-surface-sunken text-ink">
      ...
      <section className="relative overflow-hidden border-b border-line/50 pt-32 pb-24 md:pb-28">
        ...
        <div className="mx-auto max-w-7xl px-6 relative z-10">
          <div className="mx-auto max-w-4xl text-center">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-line/60 bg-white/[0.03] px-3.5 py-1">
              <span className="text-[11px] font-medium uppercase tracking-[0.15em] text-ink-secondary">Platform Architecture</span>
            </div>
            ...
            {layers.map((layer, i) => (
              ...
              >
                <div className="hidden md:flex absolute left-[14px] top-1/2 -translate-y-1/2 w-[34px] h-[34px] rounded-full bg-slate-900 border-2 border-slate-700 group-hover:border-cyan-500/50 transition-colors z-10 items-center justify-center">
                  <span className="text-[10px] font-bold tabular-nums text-ink-secondary group-hover:text-cyan-400 transition-colors">{String(i + 1).padStart(2, "0")}</span>
                </div>
                <div className="rounded-2xl border border-line/60 bg-gradient-to-br from-slate-900/60 to-slate-900/30 p-6 lg:p-8 transition-all duration-300 hover:-translate-y-0.5 hover:border-slate-700/60">
                  <div className="grid lg:grid-cols-[auto_1fr] gap-6 items-start">
                    <div className={`hidden lg:flex w-16 h-16 rounded-2xl bg-gradient-to-br ${layer.gradient} items-center justify-center shrink-0 border border-subtle`}>
                      <layer.icon className={`h-7 w-7 ${layer.iconColor}`} />
                    </div>
                    <div>
                      <div className="flex items-center gap-3 mb-4">
                        <h2 className="text-xl lg:text-2xl font-bold text-ink">{layer.title}</h2>
                        <div className="h-px flex-1 bg-gradient-to-r from-slate-700 to-transparent" />
                      </div>
                      <p className="text-sm text-ink-secondary leading-relaxed mb-6">{layer.desc}</p>
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        {layer.capabilities.map((cap) => (
                          <div key={cap} className="flex items-center gap-2 text-xs text-ink-muted">
      ...
      {/* CTA */}
      <section className="py-24 text-center border-t border-line/50">
        <div className="mx-auto max-w-3xl px-6">
          <h2 className="text-3xl lg:text-4xl font-bold text-ink mb-4">Ready to integrate with the knowledge graph?</h2>
          <p className="text-[17px] text-ink-secondary mb-8 max-w-md mx-auto">Product documentation and API status in one place.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link href="/api-access" className="inline-flex items-center justify-center gap-2 rounded-xl border border-line bg-slate-900/40 px-7 py-3.5 text-sm font-medium text-ink-secondary transition-all duration-150 hover:bg-slate-900 hover:border-slate-700 hover:text-ink hover:scale-[1.02] active:scale-100">
              API Status
            </Link>
          </div>
        </div>
