      ...
      <section className="py-24">
        <div className="mx-auto max-w-4xl px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
            ...
            <h1 className="text-5xl font-bold text-ink mb-6">Security at Patorbit</h1>
          ...
          <div className="mb-12">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-sm text-ink-muted">Working today</span>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              {implemented.map((item, i) => (
                <motion.div
                  ...
                  className="rounded-xl border border-line bg-slate-900/60 p-6 hover:border-emerald-500/30 transition-all"
                >
          ...
          <div className="mb-12">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-sm text-ink-muted">Being developed — not available yet</span>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              {inProgress.map((item, i) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: i * 0.05 }}
                  className="rounded-xl border border-line bg-slate-900/40 p-6"
                >
                  <h3 className="text-lg font-semibold text-ink mb-2">{item.title}</h3>
                  <p className="text-sm text-ink-secondary">{item.desc}</p>
                </motion.div>
          ...
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.04] p-6 mb-12">
            <h2 className="text-lg font-semibold text-ink mb-2">Certifications</h2>
            <p className="text-sm text-ink-secondary">
              Patorbit holds <strong className="text-ink">no security certifications</strong> today.
          ...
          {/* Data practices */}
          <div className="rounded-xl border border-line bg-slate-900/60 p-6 mb-12">
            <h2 className="text-lg font-semibold text-ink mb-4">Your data</h2>
            <ul className="space-y-3 text-sm text-ink-secondary">
              <li><strong className="text-ink-secondary">Retention:</strong> Your data stays while your account exists. Delete your account and it&apos;s removed.</li>
              <li><strong className="text-ink-secondary">Export:</strong> Download a full JSON copy of your data from settings at any time.</li>
              <li><strong className="text-ink-secondary">AI processing:</strong> Content you send to AI features is processed by our configured AI provider to generate suggestions. We don&apos;t sell your data.</li>
              <li><strong className="text-ink-secondary">Subprocessors:</strong> Hosting (Vercel), database (Neon/Postgres), payments (Razorpay), sign-in (GitHub, LinkedIn), AI provider (per configuration).</li>
            </ul>
          </div>
          ...
          <div className="text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <h2 className="text-3xl font-bold text-ink mb-4">Questions about security?</h2>
