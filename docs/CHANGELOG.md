...
### Added
- **Resume readability + bullet completeness (M2)** — the resume now renders at human-readable type by default: body/bullets 14px (≈10.5pt), section headings 15px, company/job-title 14–15px, dates and meta 12.5px, name ~30px; summary/description paragraphs follow body size. Typography is driven by one type scale: new `rs(px)` helper (`src/lib/resume-design-system/type-scale.ts`) emits `calc(var(--rs-type, 1) * Npx)` so every template size scales from a single CSS variable that survives `serializePage()` (the preview, print/PDF target and gallery share it). All ~200 hardcoded template `fontSize` values were remapped into `rs()` tiers; Tailwind `text-xs`/`text-sm` classes inside `FormattedDescription`/`SocialLinks` were replaced with `rs()` inline sizes (a fixed 12px class was overriding the readable parent size).
- **Typography controls in the Customize panel** — Text size (Small/Comfortable/Large → `--rs-type` 0.95/1/1.1), Heading size (Compact/Standard/Prominent → `headingScale` tier 0.9/1/1.15 applied to name + section headings in both CSS and DOCX), plus existing Font family and Line height (Tight/Standard/Relaxed). One coherent typography model — no per-section controls.
- **DOCX content parity** — `/api/export-docx` now renders experience/project `bulletPoints` (previously dropped entirely — every bullet was silently missing from Word exports), education GPA/honors/location, and project role/dates. Verified end-to-end against a real authenticated request.

### Fixed
- **4-bullet cap in resume previews** — the content plan (`content-plan.ts`) hard-capped `maxBulletsPerItem` at 4 (page 1) / 6 (page 2) for every plan, so `materializePlan` truncated roles to 4 bullets even without a target job. The cap now applies only to job-aware plans; default plans never truncate. Page-2 targeting is content-aware (`roleCount >= 4 || overBulletBudget`).
- **Microscopic template typography** — template components hardcoded inline font sizes as small as 8–10px (≈6–7.5pt); the `fontScale` zoom was stripped by `serializePage()` so scaling never reached the real preview/print. Replaced with the `rs()` type-scale architecture above; content-bearing chips/badges raised from 11px to 12.5px (decorative bullet glyphs stay small on purpose).
- **Orphaned section headings at page bottoms** — `PaginatedResumeSheet` could split between a heading and its content during reflow (Education stranded on page 1 while its entry moved to page 2). `keepsWithNext()` now also treats a single-child wrapper containing a heading (SECTION → DIV → H2), and `reflowPages` never cuts immediately after a keep-with-next block: the heading travels with its first entry; a heading-only page only occurs when the block cannot fit a fresh page either. Also: `hasAtomicDescendant` reads computed `break-inside: avoid` inline styles (one `getComputedStyle` per node) so entries marked inline count as atomic units instead of moving whole sections.
...
- **Canonical A4 page-frame + pagination** (`src/lib/resume-design-system/page-frame.ts`, `src/components/resume/PaginatedResumeSheet.tsx`) — real DOM pagination across actual A4 pages with consistent safe top/bottom space; the Gallery, Professional Preview, and PDF export all render through the same paginated DOM. Templates remain untouched; `break-inside-avoid` applies only to small semantic blocks.
- **Resume import hardening** — the import review → Apply flow now merges imported content into the user's real resume while preserving `templateId`, `claims`, `careerStage`, and identity; imports without a real template never reset the user's template (regression tests added).
- **Master Architecture documentation** (`docs/MASTER_ARCHITECTURE.md`) — project-direction document with product vision, current-vs-future status, claim/evidence/verification/conflict/trust models, platform architecture, and the 6-phase identity/trust roadmap.
